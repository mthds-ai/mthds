...
- **`Makefile` `ROOT_ROBOTS_TXT`** — The authoritative robots.txt assembled into `site-output/` by `docs-assemble-site`. Allows `/latest/`, `/sitemap.xml` and the two `llms` files, and disallows the archived version directories, `/pre-release/` and `/404.html` by name. It carries no blanket `Disallow: /`, so an archived version directory is only excluded if it is listed — and robots.txt has no numeric wildcard, so the exclusions run one per major release line (`/0.`, `/2.`). A new line needs a `Disallow` here **and** a matching `noindex, follow` header block in `vercel.json`; miss either and that version ships as an indexable duplicate of `/latest/`. Points `Sitemap:` to the root `https://mthds.ai/sitemap.xml`.
