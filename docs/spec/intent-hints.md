...
- On a **plural site** — an input slot with `[]` or `[N]` multiplicity, or a structure field with `type = "list"` — applicability is judged against the item as if it stood alone: a slot's item is its concept; a list field's item is a field of its `item_type`, which for `item_type = "concept"` is the concept its `item_concept_ref` names. An applicable word refines the presentation of each item. The collection's own presentation follows from its semantics.
