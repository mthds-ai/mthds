...
|--------|------|----------|-------------|
| `mthds_version` | string | Yes | The [MTHDS standard version](./versioning.md) the crate was normalized against, so native expansion (below) is self-describing. |
...
- Entries contributed by a **dependency** (direct or transitive) are keyed by that dependency's package address, joined with `::`: `github.com/acme/legal-tools::legal.contracts.ContractClause`, `github.com/mthds/scoring-lib::scoring.compute_weighted_score`. The address is the declaring package's **full address** — its manifest `address`, or `address + "/" + name` for a package in a library repository (see [Distribution: Locating a Package Inside a Clone](../packages/distribution.md#locating-a-package-inside-a-clone)) — so two sibling packages that share a repository `address` keep distinct prefixes.
...
The halves of that promise have standard-owned artifacts of their own, so a consumer need not re-derive them: [Pipe I/O Contracts](./pipe-io-contracts.md) states, per pipe, the concept, presence, multiplicity and JSON Schema of every input and the concept and multiplicity of the output — what "register a correct tool" needs; the [Input-Form Descriptor](./input-form-descriptor.md) states the ordered, kind-discriminated presentation view of the same inputs — what "render a correct input form" needs; and the [Output-Form Descriptor](./output-form-descriptor.md) states that same view of what each pipe resolves to — what showing or typing a result needs. All are projections of a resolved library and are derivable from a crate with a JSON parser and nothing else, which is sufficiency demonstrated rather than asserted. The one place a normalized crate is a lossy input is the descriptors' `refines` chain, which needs the refinement links [step 3](#3-flatten-refinement) deliberately flattens; the input-form descriptor page states what a crate-only producer reports there, and the output-form descriptor binds that requirement unchanged.
...
- [Pipe I/O Contracts](./pipe-io-contracts.md), the [Input-Form Descriptor](./input-form-descriptor.md) and the [Output-Form Descriptor](./output-form-descriptor.md) are the standard-owned projections of a resolved library — the machine contract, and the presentation views of what a pipe takes and of what it resolves to.
