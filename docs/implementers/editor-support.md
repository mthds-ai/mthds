...
3. **Pipe type values** — string values that match the ten pipe type names (`PipeLLM`, `PipeFunc`, etc.) in the `type` field of pipe sections.
...
- All ten pipe types with their specific fields
