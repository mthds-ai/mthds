...
- The workspace descriptor spec's "reserved hint slot" section can then link to the accepted `spec/intent-hints.md` page as the vocabulary owner — and drop `choice` from the vocabulary it lists, which this session rejected. That same edit should state that an applicable `intent` word is an *input* to the deriver's `kind` derivation (the no-hint default), never a second answer competing with a derived `kind`.
- The hints spec treats a description-only concept (neither `structure` nor `refines`) as text-valued, as the reference implementation does (it refines `native.Text` at run time); Library Crate step 6 still promotes the string-shorthand form to a 'structureless' concept with no `refines`. Aligning step 6 with that reading is a follow-up on the crate spec.
- The standard's own validate surfaces (`docs/spec/protocol.md`, the OpenAPI document, `docs/spec/cli-io-contract.md`) define no advisory warnings member, so the SHOULD-warn rules in the hints spec can only be honored through an implementation's own diagnostics channel today. Adding an advisory channel to the standard's protocol is its own follow-up.
