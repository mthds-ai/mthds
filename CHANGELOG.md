...
- **Output-Form Descriptor** (`spec/output-form-descriptor.md`): New specification for the presentation view of what a pipe resolves to — one field descriptor per pipe, stating its output's kind, nesting and constraints, so that showing a result or generating a type for it needs no schema heuristics. It reuses the input-form descriptor's field descriptor unchanged and states only what an output position changes: no `presence` and no `gating`, the fixed node name `output`, `required` always `true` with possible absence left to the contract's `optional`, and the plural wrap that makes a `Concept[]` output a `list` node whose `item` is the element.
- **`output_form` is a recommended extension field of the validate response**, on the same terms as `pipe_io_contracts` and `input_form`: a runner reporting it reports this shape under this name, and one omitting it is still conformant. No base field and no route changes, so the protocol version stays where it is.
...
- **Recommended extension fields do not move the protocol version**: `spec/versioning.md` now says what the protocol number covers — the base surface, meaning the routes and response fields of the normative OpenAPI document — and that the artifacts the standard names on the validate report's extension surface are shaped by their own pages and carried by a report that is extension-open by policy. That has been the practice since `pipe_io_contracts` and `input_form` were named, but the bump rule as written read as though naming one were a minor protocol change.
- **The sibling specifications point at the new page**: Pipe I/O Contracts names it as the presentation view over its `output`, the Input-Form Descriptor records that its field descriptor is the vocabulary of both artifacts, the Library Crate Format counts it among the standard-owned projections that demonstrate its sufficiency guarantee, and the `.mthds` File Format lists it among the artifacts that report the `[1]`-is-single rule.
