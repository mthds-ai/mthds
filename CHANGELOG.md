...
- **The published MTHDS schema describes hints and the expanded input slot form.** `docs/mthds_schema.json` moves to the schema `pipelex` v0.55.0 generates from its blueprint models: it gains the `hints` members on a concept and on a structure field, and `InputSlotBlueprint` — the expanded form in which a pipe's `inputs` value may be a table, `x = { concept = "S", hints = { intent = "prose" } }`, rather than only the string `x = "S"`. The two forms are specified as equivalent, so a schema carrying only the string arm rejected valid authoring; that is what this retires. Arriving with the same version: `PipeLLMBlueprint.templating_style`, and the removal of `LLMSetting.prompting_target` and its `PromptingTarget` definition, which the previous copy still accepted after the language had dropped them. The Intent Hints (`docs/spec/intent-hints.md`) specification-status note is corrected to match — it claimed no implementation realized the contract and that the served schema described neither addition, both of which the reference implementation has since overtaken at the authoring layer. **mthds.ai keeps serving the previous schema until the site is next deployed**; the committed copy is what changed here.
